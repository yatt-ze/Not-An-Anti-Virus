...
- Mach-O structural anomalies: unusual load commands, suspicious entitlements requested by unsigned binaries, packed/obfuscated sections, high entropy in `__TEXT`. The load-command/entitlement half is the `macho-loader-anomaly` rule (`StaticSuspicion`), built on the `macho` parser's full load-command walk (dylib/rpath paths, code-signature presence, an embedded entitlements plist recovered from the signature's SuperBlob): it flags an `LC_LOAD_DYLIB`/`LC_RPATH` path into a writable/transient location (`/tmp`, `/private/tmp`, `/var/tmp`, `/Users/Shared`, anywhere under `/Users/`, or a hidden directory component — never the normal `@executable_path`/`@loader_path`/`@rpath` relative forms or an Apple system location) and a handful of "exempt me from a platform protection" entitlements (`com.apple.security.cs.disable-library-validation`, `...allow-dyld-environment-variables`, `...disable-executable-page-protection`, `com.apple.security.get-task-allow`), weighted higher when the binary carries no code signature at all. Both anomalies are corroboration-only and share a capped weight, so the rule alone never reaches the §5.1 high-severity threshold. The entitlements blob sits near the end of the file (in `__LINKEDIT`), exactly where an 8 MiB bounded capture (§3) is most likely to have been cut off — a signed binary with no entitlements *recovered* is only read as "no entitlements" when the capture wasn't truncated; a truncated capture reports `RuleOutcome::NotApplicable` for that half instead of a false-clean read (§10/§11.8), while a real anomaly already found in the load paths still fires. A fat/universal binary is judged on **all** its architecture slices, not just the first — the rule iterates `macho::parse_all`, unions the load-path anomalies across slices, and scores each slice's entitlements on its own signing status, so a malicious arm64 slice can't hide behind a benign x86_64 one. High/obfuscated-section entropy is the separate `high-entropy-content` rule (§5.1), which scores `__TEXT`'s entropy using the same parser's `text_range`.
